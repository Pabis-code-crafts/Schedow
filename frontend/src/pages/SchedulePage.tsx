...
export function SchedulePage() {
  ...
  const [emptyCellPopoverOpen, setEmptyCellPopoverOpen] = useState(false);
  ...
  useEffect(() => {
    if (!emptyCellPopoverOpen || selection.type !== 'shift' || !parseEmptyId(selection.shiftId)) return;
  ...
  }, [emptyCellPopoverOpen, selection]);
  ...
  return (
    <>
      <Stack spacing={2.5}>
        <Stack alignItems={{ xs: 'stretch', lg: 'flex-start' }} direction={{ xs: 'column', lg: 'row' }} justifyContent="space-between" spacing={2.5}>
          <Box sx={{ maxWidth: 560 }}>
            <Typography color="primary.dark" fontWeight={800} letterSpacing={0.4} textTransform="uppercase" variant="caption">
              Schedule workspace
            </Typography>
            <Typography component="h1" sx={{ fontSize: { xs: 30, md: 40 }, fontWeight: 850, lineHeight: 1.15, mt: 1 }}>
              Shift recommendations with <Box component="span" sx={{ color: 'primary.main' }}>Schedow Copilot</Box>
            </Typography>
            <Typography color="text.secondary" sx={{ maxWidth: 640, mt: 1.5 }} variant="body1">
              Plan smarter with AI-powered shift recommendations, workload balancing, and real-time scheduling insights in one calm workspace.
            </Typography>
          </Box>
          {isLoading || isFetching ? <MetricSkeleton /> : <MetricCards metrics={metrics} />}
        </Stack>

        <Stack alignItems="flex-start" direction={{ xs: 'column', xl: 'row' }} spacing={2.5}>
          <Box sx={{ flex: 1, minWidth: 0, width: '100%' }}>
            <Stack spacing={2}>
              ...
              <AppCard sx={{ borderRadius: 3, boxShadow: 2, overflow: 'visible', width: '100%' }}>
                {isLoading || isFetching ? <GridSkeleton /> : isError ? <Stack spacing={2}><Alert severity="error">Could not load the schedule assignments. {getApiErrorMessage(error)}</Alert><Button onClick={() => void refetch()} variant="contained">Retry</Button></Stack> : schedule && schedule.shiftTemplates.length > 0 ? <Stack spacing={2}><ActionBar onAdd={openSelectedAssignment} /><ScheduleGrid activeAction={activeAction} emptyCellPopoverOpen={emptyCellPopoverOpen} schedule={schedule} selection={selection} onAssign={(day, template) => { createAssignment.reset(); setActiveAction('assign'); setEmptyCellPopoverOpen(false); setSelection({ type: 'none' }); setDraft({ day, mode: 'create', template }); }} onMenu={(anchorEl, context) => setMenu({ anchorEl, context })} onRecommend={openRecommend} onSelectDay={(dayId) => { setActiveAction(null); setEmptyCellPopoverOpen(false); setSelection((current) => current.type === 'day' && current.dayId === dayId ? { type: 'none' } : { type: 'day', dayId }); setAiOpen(true); }} onSelectEmpty={(day, template) => { const id = emptyId(day.id, template.id); setActiveAction(null); setSelection((current) => { if (current.type === 'shift' && current.shiftId === id) { setEmptyCellPopoverOpen(false); return { type: 'none' }; } setEmptyCellPopoverOpen(true); return { type: 'shift', shiftId: id }; }); setAiOpen(true); }} onSelectShift={(_, shift) => { setActiveAction(null); setEmptyCellPopoverOpen(false); setSelection((current) => current.type === 'shift' && current.shiftId === shift.id ? { type: 'none' } : { type: 'shift', shiftId: shift.id }); setAiOpen(true); }} /><ScheduleLegend /></Stack> : <EmptyState action={<Button onClick={() => void refetch()} variant="outlined">Refresh</Button>} icon={<CalendarMonthRoundedIcon />} title="No assignments for this week" description="Once the backend returns shift assignments, this workspace will render the week dynamically from that data." />}
              </AppCard>
            </Stack>
          </Box>
      ...
      {!aiOpen && schedule ? <Tooltip title="Open Schedow Copilot"><Button color="primary" onClick={() => setAiOpen(true)} size="large" startIcon={<AutoAwesomeRoundedIcon />} sx={{ bottom: { xs: 20, sm: 28 }, boxShadow: 5, position: 'fixed', right: { xs: 20, sm: 32 }, zIndex: (theme) => theme.zIndex.drawer - 1 }} variant="contained">Schedow Copilot</Button></Tooltip> : null}
...
function MetricCards({ metrics }: { metrics: ReturnType<typeof getMetrics> }) {
  const cards = [
    { helper: 'Needs coverage', icon: <AddRoundedIcon />, label: 'Open shifts', tone: 'warning' as const, value: metrics.open },
    { helper: 'Smart suggestions', icon: <AutoAwesomeRoundedIcon />, label: 'AI signals', tone: 'info' as const, value: metrics.ai },
    { helper: 'This week', icon: <GroupsRoundedIcon />, label: 'Assigned shifts', tone: 'success' as const, value: metrics.assigned },
  ];

  return (
    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ minWidth: { lg: 560 }, width: { xs: '100%', lg: 'auto' } }}>
      {cards.map((card) => (
        <AppCard key={card.label} sx={{ flex: 1, minWidth: { sm: 168 }, p: 0 }}>
          <Stack spacing={1.4}>
            <Stack alignItems="flex-start" direction="row" justifyContent="space-between" spacing={2}>
              <Box>
                <Typography color="text.secondary" variant="caption">{card.label}</Typography>
                <Typography sx={{ fontSize: 30, fontWeight: 850, lineHeight: 1.1, mt: 0.5 }}>{card.value}</Typography>
              </Box>
              <Box sx={{ alignItems: 'center', bgcolor: (theme) => alpha(theme.palette[card.tone].main, 0.18), borderRadius: '50%', color: `${card.tone}.dark`, display: 'flex', height: 36, justifyContent: 'center', width: 36 }}>
                {card.icon}
              </Box>
            </Stack>
            <Typography color="text.secondary" variant="caption">{card.helper}</Typography>
          </Stack>
        </AppCard>
      ))}
    </Stack>
...
function WeekToolbar({ fetching, label, onDate, onMove, onToday, onWeek, value, weekSelected }: { fetching: boolean; label: string; value: string; weekSelected: boolean; onDate: (date: string) => void; onMove: (days: number) => void; onToday: () => void; onWeek: () => void }) {
  return <Box sx={{ bgcolor: (theme) => alpha(theme.palette.background.paper, 0.76), border: 1, borderColor: 'divider', borderRadius: 4, boxShadow: 1, px: { xs: 2, md: 2.5 }, py: 2 }}><Stack alignItems={{ xs: 'stretch', lg: 'center' }} direction={{ xs: 'column', lg: 'row' }} justifyContent="space-between" spacing={2}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}><Button disabled={fetching} onClick={() => onMove(-7)} startIcon={<ChevronLeftRoundedIcon />} variant="outlined">Previous Week</Button><Button disabled={fetching} endIcon={<ChevronRightRoundedIcon />} onClick={() => onMove(7)} variant="outlined">Next Week</Button><Button disabled={fetching} onClick={onToday}>Today</Button></Stack><Box sx={{ textAlign: { xs: 'left', lg: 'center' } }}><Typography color="text.secondary" variant="caption">Viewing</Typography><Typography sx={{ fontSize: { xs: 20, md: 24 }, fontWeight: 800 }}>{label}</Typography></Box><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.25}><TextField InputLabelProps={{ shrink: true }} disabled={fetching} label="Pick date" onChange={(e) => onDate(e.target.value)} size="small" type="date" value={value} /><Button disabled={fetching} onClick={onWeek} startIcon={<CalendarMonthRoundedIcon />} variant={weekSelected ? 'contained' : 'outlined'}>Select Week</Button></Stack></Stack></Box>;
}
function ActionBar({ onAdd }: { onAdd: () => void }) {
  return <Stack alignItems={{ xs: 'stretch', sm: 'center' }} direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={1.5}><Box><Typography sx={{ fontWeight: 800 }} variant="subtitle1">Weekly schedule</Typography><Typography color="text.secondary" variant="caption">Click a cell to assign, review or ask Copilot for recommendations.</Typography></Box><Button onClick={onAdd} size="small" startIcon={<AddRoundedIcon />} variant="contained">Add Assignment</Button></Stack>;
}

function ScheduleGrid({ activeAction, emptyCellPopoverOpen, onAssign, onMenu, onRecommend, onSelectDay, onSelectEmpty, onSelectShift, schedule, selection }: { activeAction: 'assign' | 'recommend' | null; emptyCellPopoverOpen: boolean; schedule: WeeklySchedule; selection: Selection; onAssign: (day: WeekDay, template: ShiftTemplate) => void; onMenu: (anchorEl: HTMLElement, context: CellContext) => void; onRecommend: (day: WeekDay, template: ShiftTemplate) => void; onSelectDay: (dayId: string) => void; onSelectEmpty: (day: WeekDay, template: ShiftTemplate) => void; onSelectShift: (event: MouseEvent, shift: ScheduledShift) => void }) {
  const dividerColor = (theme: { palette: { divider: string } }) => alpha(theme.palette.divider, 0.72);

  return <Box sx={{ overflowX: { xs: 'auto', lg: 'visible' }, pb: 0.5 }}><Box sx={{ bgcolor: 'background.paper', border: 1, borderColor: (theme) => alpha(theme.palette.divider, 0.92), borderRadius: 3, display: 'grid', gridTemplateColumns: { xs: '112px repeat(7, minmax(126px, 1fr))', lg: '124px repeat(7, minmax(0, 1fr))' }, minWidth: { xs: 1040, lg: 0 }, overflow: 'hidden', '& > *': { borderBottom: 1, borderColor: dividerColor, borderRight: 1 }, '& > :nth-of-type(8n)': { borderRight: 0 }, '& > :nth-last-of-type(-n+8)': { borderBottom: 0 } }}><Box sx={{ bgcolor: (theme) => alpha(theme.palette.grey[50], 0.92), p: 1.25 }}><Typography fontWeight={850} letterSpacing={0.3} textTransform="uppercase" variant="caption">Shift</Typography><Typography color="text.secondary" display="block" variant="caption">Time</Typography></Box>{schedule.days.map((day) => <DayHeader day={day} key={day.id} selected={selection.type === 'day' ? selection.dayId === day.id : selection.type === 'week'} onClick={() => onSelectDay(day.id)} />)}{schedule.shiftTemplates.map((template) => <ScheduleRow activeAction={activeAction} emptyCellPopoverOpen={emptyCellPopoverOpen} key={template.id} onAssign={onAssign} onMenu={onMenu} onRecommend={onRecommend} onSelectEmpty={onSelectEmpty} onSelectShift={onSelectShift} schedule={schedule} selection={selection} template={template} />)}</Box></Box>;
}

function ScheduleRow({ activeAction, emptyCellPopoverOpen, onAssign, onMenu, onRecommend, onSelectEmpty, onSelectShift, schedule, selection, template }: { activeAction: 'assign' | 'recommend' | null; emptyCellPopoverOpen: boolean; schedule: WeeklySchedule; selection: Selection; template: ShiftTemplate; onAssign: (day: WeekDay, template: ShiftTemplate) => void; onMenu: (anchorEl: HTMLElement, context: CellContext) => void; onRecommend: (day: WeekDay, template: ShiftTemplate) => void; onSelectEmpty: (day: WeekDay, template: ShiftTemplate) => void; onSelectShift: (event: MouseEvent, shift: ScheduledShift) => void }) {
  return <><Box sx={{ alignSelf: 'stretch', bgcolor: (theme) => alpha(theme.palette.grey[50], 0.78), minHeight: 68, p: 1.25 }}><Typography noWrap variant="subtitle2">{template.name}</Typography><Typography color="text.secondary" mt={0.25} variant="caption">{shiftTime(template)}</Typography></Box>{schedule.days.map((day) => { const shift = getShift(schedule, day.id, template.id); return shift ? <ShiftCell key={shift.id} selected={isShiftSelected(schedule, selection, shift.id)} shift={shift} onClick={(event) => onSelectShift(event, shift)} onMenu={(anchorEl) => onMenu(anchorEl, { day, shift, template })} /> : <EmptyCell activeAction={isEmptySelected(selection, day.id, template.id) ? activeAction : null} day={day} key={`${day.id}-${template.id}`} popoverOpen={isEmptySelected(selection, day.id, template.id) && emptyCellPopoverOpen} selected={isEmptySelected(selection, day.id, template.id)} template={template} onAssign={() => onAssign(day, template)} onClick={() => onSelectEmpty(day, template)} onRecommend={() => onRecommend(day, template)} />; })}</>;
}

function DayHeader({ day, onClick, selected }: { day: WeekDay; selected: boolean; onClick: () => void }) {
  return <ButtonBase onClick={onClick} sx={{ display: 'block', height: '100%', textAlign: 'center' }}><Box sx={{ bgcolor: selected ? (theme) => alpha(theme.palette.primary.main, 0.1) : (theme) => alpha(theme.palette.grey[50], 0.92), boxShadow: selected ? (theme) => `inset 0 -2px 0 ${theme.palette.primary.main}` : 'none', color: selected ? 'primary.dark' : 'text.primary', minHeight: 58, p: 1, '&:hover': { bgcolor: (theme) => alpha(theme.palette.primary.main, 0.08) } }}><Typography noWrap sx={{ fontWeight: 800 }} variant="subtitle2">{day.label}</Typography><Typography color="text.secondary" variant="caption">{day.date}</Typography></Box></ButtonBase>;
}
function ShiftCell({ onClick, onMenu, selected, shift }: { shift: ScheduledShift; selected: boolean; onClick: (event: MouseEvent) => void; onMenu: (anchorEl: HTMLElement) => void }) {
  ...
  const assigned = shift.assignedWorkerCount || shift.assignedWorkers.length;
  const complete = assigned >= required;
  const tone = complete ? 'success' : 'warning';

  return <ButtonBase onClick={onClick} sx={{ display: 'block', height: '100%', textAlign: 'left', width: '100%' }}><Box sx={{ alignItems: 'center', bgcolor: selected ? (theme) => alpha(theme.palette.primary.main, 0.055) : 'background.paper', boxShadow: selected ? (theme) => `inset 0 0 0 1px ${alpha(theme.palette.primary.main, 0.3)}` : 'none', display: 'flex', minHeight: 66, p: 0.8, transition: (theme) => theme.transitions.create(['background-color']), '&:hover': { bgcolor: (theme) => alpha(theme.palette.primary.main, 0.026), '& .cell-menu-button': { opacity: 1 } } }}><Box sx={{ alignItems: 'center', bgcolor: (theme) => alpha(theme.palette[tone].main, selected ? 0.13 : 0.075), borderRadius: 2, display: 'grid', gap: 0.9, gridTemplateColumns: '3px minmax(0, 1fr) 26px', minHeight: 44, width: '100%' }}><Box sx={{ alignSelf: 'stretch', bgcolor: `${tone}.main`, borderRadius: '2px 0 0 2px' }} /><Stack direction="row" spacing={0.8} sx={{ alignItems: 'center', minWidth: 0, py: 0.7 }}><Avatar sx={{ bgcolor: (theme) => alpha(theme.palette[tone].main, 0.2), color: `${tone}.dark`, flexShrink: 0, fontSize: 11, fontWeight: 800, height: 26, width: 26 }}>{worker?.initials ?? worker?.name?.[0] ?? 'W'}</Avatar><Typography noWrap sx={{ fontWeight: 750, lineHeight: 1.2, minWidth: 0 }} variant="caption">{worker?.name ?? 'Worker'}</Typography></Stack><IconButton className="cell-menu-button" aria-label="Open cell actions" onClick={(event) => { event.stopPropagation(); onMenu(event.currentTarget); }} size="small" sx={{ alignSelf: 'center', color: 'text.secondary', height: 24, justifySelf: 'center', opacity: { xs: 1, md: 0.38 }, transition: (theme) => theme.transitions.create(['opacity', 'background-color']), width: 24 }}><MoreHorizRoundedIcon sx={{ fontSize: 17 }} /></IconButton></Box></Box></ButtonBase>;
}

function EmptyCell({ activeAction, day, onAssign, onClick, onRecommend, popoverOpen, selected, template }: { activeAction: 'assign' | 'recommend' | null; day: WeekDay; popoverOpen: boolean; selected: boolean; template: ShiftTemplate; onAssign: () => void; onClick: () => void; onRecommend: () => void }) {
  const ref = useRef<HTMLButtonElement | null>(null);
  return <><ButtonBase ref={ref} data-empty-cell-trigger={selected ? 'selected' : undefined} aria-label={`Select ${template.name} on ${day.fullLabel}`} onClick={onClick} sx={{ display: 'block', height: '100%', overflow: 'visible', textAlign: 'left', width: '100%' }}><Box sx={{ alignItems: 'center', bgcolor: selected ? (theme) => alpha(theme.palette.primary.main, 0.04) : 'background.paper', boxShadow: selected ? (theme) => `inset 0 0 0 1px ${alpha(theme.palette.primary.main, 0.3)}` : 'none', color: 'primary.main', display: 'flex', justifyContent: 'center', minHeight: 66, px: 1, py: 0.8, transition: (theme) => theme.transitions.create(['background-color', 'color']), '&:hover': { bgcolor: (theme) => alpha(theme.palette.primary.main, 0.024), color: 'primary.dark', '& .empty-cell-plus': { opacity: 1, transform: 'scale(1)' } } }}><AddRoundedIcon className="empty-cell-plus" sx={{ fontSize: 18, opacity: selected ? 0.95 : 0.64, transform: selected ? 'scale(1)' : 'scale(0.94)', transition: (theme) => theme.transitions.create(['opacity', 'transform']) }} /></Box></ButtonBase><Popper anchorEl={ref.current} disablePortal={false} modifiers={[{ name: 'offset', options: { offset: [0, 12] } }, { name: 'flip', options: { fallbackPlacements: ['left-start', 'right-end', 'left-end', 'bottom-start', 'top-start'], padding: 16 } }, { name: 'preventOverflow', options: { boundary: 'viewport', padding: 16, rootBoundary: 'viewport' } }]} open={popoverOpen} placement="right-start" sx={{ zIndex: (theme) => theme.zIndex.modal + 1 }}><Box data-empty-cell-popover="true" onClick={(event) => event.stopPropagation()} sx={{ bgcolor: 'background.paper', border: 1, borderColor: (theme) => alpha(theme.palette.primary.main, 0.18), borderRadius: 2, boxShadow: 4, minWidth: 224, p: 1, width: 224 }}><Stack spacing={0.75}><Button fullWidth onClick={(event) => { event.stopPropagation(); onAssign(); }} size="small" startIcon={<AddRoundedIcon />} sx={{ justifyContent: 'flex-start', whiteSpace: 'nowrap' }} variant={activeAction === 'assign' ? 'contained' : 'outlined'}>Assign Shift</Button><Button fullWidth onClick={(event) => { event.stopPropagation(); onRecommend(); }} size="small" startIcon={<AutoAwesomeRoundedIcon />} sx={{ justifyContent: 'flex-start', whiteSpace: 'nowrap' }} variant={activeAction === 'recommend' ? 'contained' : 'outlined'}>Recommend Worker</Button></Stack></Box></Popper></>;
}

function ScheduleLegend() {
  const items = [
    { color: 'success.main', label: 'Fully Assigned' },
    { color: 'warning.main', label: 'Partially Assigned' },
    { color: 'secondary.main', label: 'Open' },
  ];

  return <Stack alignItems={{ xs: 'flex-start', sm: 'center' }} direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={1.5} sx={{ pt: 0.5 }}><Stack direction="row" flexWrap="wrap" gap={2}>{items.map((item) => <Stack alignItems="center" direction="row" key={item.label} spacing={0.75}><Box sx={{ bgcolor: item.color, borderRadius: '50%', height: 10, width: 10 }} /><Typography color="text.secondary" variant="caption">{item.label}</Typography></Stack>)}</Stack><Typography color="text.secondary" variant="caption">Click on a cell to assign or view details</Typography></Stack>;
}
...
function AiPanel({ isApproving, onApproveAssignment, onClose, schedule, selection }: { isApproving: boolean; schedule: WeeklySchedule; selection: Selection; onApproveAssignment: (proposal: AiActionProposal) => Promise<void>; onClose: () => void }) {
  ...
  const send = async (message: string) => {
    ...
    try {
      ...
      setMessages((current) => [...current, { id: crypto.randomUUID(), role: 'assistant', content: 'Schedow Copilot could not respond.' }]);
    }
  };
  ...
  return <AppCard sx={{ flexShrink: 0, position: { xl: 'sticky' }, top: { xl: 96 }, width: { xs: '100%', xl: 330 } }}><Stack spacing={2.25}><Stack alignItems="center" direction="row" justifyContent="space-between"><Stack alignItems="center" direction="row" spacing={1.25}><Box sx={{ alignItems: 'center', bgcolor: 'primary.main', borderRadius: '50%', color: 'primary.contrastText', display: 'flex', height: 40, justifyContent: 'center', width: 40 }}><AutoAwesomeRoundedIcon fontSize="small" /></Box><Box><Typography sx={{ fontWeight: 850 }} variant="subtitle1">Schedow Copilot</Typography><Typography color="text.secondary" variant="caption">Your scheduling assistant</Typography></Box></Stack><IconButton aria-label="Close Copilot" onClick={onClose} size="small"><CloseRoundedIcon /></IconButton></Stack><Box sx={{ maxHeight: { xs: 360, xl: 'calc(100vh - 420px)' }, minHeight: 220, overflowY: 'auto' }}>{messages.length ? <Stack spacing={1.25}>{messages.map((message) => <ChatMessageBubble key={message.id} message={message} />)}{pendingAction ? <AiApprovalCard action={pendingAction} error={approvalError} isApproving={isApproving} schedule={schedule} onApprove={() => void approve()} /> : null}{approvedMessage ? <Alert severity="success">{approvedMessage}</Alert> : null}{chat.isPending ? <CircularProgress size={18} /> : null}</Stack> : <Stack spacing={1.1}><Typography color="text.secondary" variant="caption">Suggested prompts</Typography>{context.prompts.map((prompt) => <Button fullWidth key={prompt} onClick={() => void send(prompt)} sx={{ borderColor: 'divider', justifyContent: 'flex-start', minHeight: 42, textAlign: 'left' }} variant="outlined">{prompt}</Button>)}</Stack>}</Box><Stack spacing={1.25}><Typography color="text.secondary" variant="caption">Active context</Typography><Box sx={{ bgcolor: (theme) => alpha(theme.palette.grey[100], 0.78), border: 1, borderColor: 'divider', borderRadius: 999, px: 1.75, py: 1 }}><Typography noWrap variant="subtitle2">{context.label}</Typography></Box><TextField fullWidth minRows={3} multiline onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(draft); } }} placeholder="Message Schedow Copilot..." size="small" value={draft} />{chat.error ? <Alert severity="error">{getApiErrorMessage(chat.error)}</Alert> : null}<Button disabled={!draft.trim() || chat.isPending} onClick={() => void send(draft)} variant="contained">Send</Button><Typography color="text.secondary" sx={{ textAlign: 'center' }} variant="caption">Copilot uses your schedule data to generate smart suggestions.</Typography></Stack></Stack></AppCard>;
}
